...
@click.option("--sequences", default=".", help="The directory or to search for sequences in")
@click.option("--sequence-ids-path", default=None, help="Path to the file containing the sequence-ids to search for in sequences")
...
@click.option("--recalculate", type=str, help="Path to txt file containing all proteins to recalculate or pass ALL to recalculate ERCs for all proteins")
@click.option("--matrix-only", is_flag=True, help="directly generates matrix for ercs and datasources only. Assumes all other steps already completed")
def main(timetree: str, sequences: str, sequence_ids_path: str, align_pair: List[Tuple[str, str]], wd: str, erc_type: str, segment: bool,
         kmer: int, slide: bool, skip_align: bool, skip_trim: bool, archive: bool, id2name: str,
         previous_run: List[str], n: int, prepare: bool, recalculate: str, matrix_only:bool):
    try_hook_uvloop()
    ...
    if matrix_only:
        arg_mods["matrix_only"] = matrix_only
    ...
    if sequence_ids_path is not None:
        sequence_ids_path = osp.abspath(sequence_ids_path)
        
    if recalculate:
        if recalculate != "ALL":
            recalculate = osp.abspath(recalculate)
            with open(recalculate, "r") as f:
                recalculate = f.read().splitlines()
        arg_mods["recalculate"] = recalculate
    ...
    if not align_pair:
        if sequence_ids_path is not None:
            with open(sequence_ids_path, "r") as f: 
                sequence_ids = list(map(lambda k: k.strip(), f.read().splitlines())) #orthoIDS used as names e.g. 59909at40674.fa
                for sequence_id in sequence_ids:
                    sequence_path = osp.join(sequences, sequence_id + ".fa")
                    if not os.path.exists(sequence_path):
                        print(f"WARNING: Sequence does not exist in reference directory")
                        continue
                    workspace.add_alignment(osp.join(sequence_path))        
        else:
