...
class ErcWorkspace:
    ...
    def __init__(self, directory: str, timetree: str, segmented: bool = False, segment_size: int = 10,
                 ...
                 cores: int = 4, archive: bool = False, prepare: bool = False, matrix_only: bool = False):
    ...
    def combo_not_exists_or_recalc(self, combo, past_trees, completed):
        #anything in align assumed to explicitly be recalculated if self.recalculate
        def get_raw_fasta_name(file_name):
            return file_name.replace(".fa.pred", "")
        def recalc(combo):
            return (self.recalculate and (get_raw_fasta_name(combo[0]) in self.recalculate)) or (self.recalculate and (get_raw_fasta_name(combo[1]) in self.recalculate))
        def not_in_trees_or_completed(combo):
            return ((combo[0] not in past_trees or combo[1] not in past_trees) and (combo[0] != combo[1]) \
            and (combo not in completed) and ((combo[1], combo[0]) not in completed))
        
        return recalc(combo) or not_in_trees_or_completed(combo)
    ...
    async def run(self):
        ...
        if(self.recalculate == "ALL"):
            self.recalculate = self.align_names
            ...
            if self.segmented:
                ...
                for combo in itertools.combinations(all_trees, 2):
                    ...
                    for chunk1, chunk2 in itertools.combinations(chunked_trees, 2):
                        if self.combo_not_exists_or_recalc(combo, past_trees, completed):
                ...
                with open(osp.join(self.directory, 'ercs.csv'), 'a') as f:
                    for res in results:
                        try:
                            ...
                            print(str(res.file1) + " | " + str(res.file2) + " | " + str(res.rho) + " | " + str(res.p))
    ...
    def export_results(self, basename: str):
        ...
        if "28286at40674" in network.nodes:
            print("28286at40674")
